...
const unsigned char kReversePi[256] = {
...
const unsigned char kB[16] = {148, 32,  133, 16, 194, 192, 1,   251,
...
int funcR(unsigned char *indata, unsigned char *outdata) {
  ...
  unsigned char sum = 0;
  for (i = 0; i < 16; ++i)
    sum ^= multiplic_polynom(*(indata + i), kB[i]);
  outdata[0] = sum;
...
int funcReverseR(unsigned char *indata, unsigned char *outdata) {
  ...
  for (i = 0; i < 16; ++i)
    sum ^= multiplic_polynom(*(tmp + i), kB[i]);
